...
# from tensorflow.keras.experimental import Ein
...
class MultiHeadAttention(Layer):
    ...
    def _build_attention(self, rank):
        ...
        self._softmax = tensorflow.keras.layers.Softmax(axis=norm_axes)
        self._dropout_layer = core.Dropout(rate=self._dropout)

    def _masker_softmax(self, inputs, mask=None, axis=-1):
        if mask is not None:
            # Since mask is 1.0 for positions we want to keep and 0.0 for
            # masked positions, this operation will create a tensor which is 0.0 for
            # positions we want to attend and -1e.9 for masked positions.
            if inputs.dtype == tensorflow.dtypes.float16:
                neg_val = tensorflow.dtypes.float16.min
            else:
                neg_val = -1e9
            adder = (1.0 - mask) * neg_val
        ...
        if isinstance(axis, (tuple, list)):
            if len(axis) > 1:
                ...
                return tensorflow.keras.layers.Softmax(axis=axis)(inputs)
        return tensorflow.keras.layers.Softmax(axis=axis)(inputs)
    ...
    def _masked_softmax(self, attention_scores, attention_mask=None):
        ...
        return self._masker_softmax(attention_scores, attention_mask)
